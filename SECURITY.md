...
- 字根煉金塔排行榜只接受已啟用作品、1–8 個英數字名稱、UUID 事件與符合遊戲計分上下限的整數成績；正式寫入限定正式站同源請求。遊戲 iframe 只以 `postMessage` 通知目前的外層播放器，不直接連線 Function，也未放寬 `allow-same-origin`。排行榜沒有帳號或伺服器逐題驗證，因此屬友善排行而非完全防作弊競賽。
