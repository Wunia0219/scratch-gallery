...
try {
  ...
  await page.getByRole('button', { name: '播放萬聖節活動說明' }).click()
  const eventFrame = page.frameLocator('dialog iframe')
  await eventFrame.locator('#launch').waitFor({ state: 'visible', timeout: 30000 })
  const eventRuntime = page.frames().find(frame => frame.url().includes('/games/halloween-activity-intro/index.html'))
  assert.ok(eventRuntime, 'Halloween activity preview frame loaded')
  assert.equal(await eventRuntime.evaluate(() => { try { return Boolean(parent.document.body) } catch { return false } }), false, 'event preview must not read parent document')
  await page.getByRole('button', { name: '關閉遊戲播放器' }).click()
  assert.equal(await page.locator('iframe').count(), 0)
  await page.goto('http://127.0.0.1:4173/students/')
  ...
  await page.goto('http://127.0.0.1:4173/teachers/')
  ...
  await readable.goto('http://127.0.0.1:4173/students/')
  assert.equal(await readable.locator('.game-card').count(), initialStudentCards)
  await readable.goto('http://127.0.0.1:4173/teachers/')
